...
TARGET_SPEED = 1100 # Forward speed slow. In case of problem, check with manual controller.
...
def talker():
    ...
    for input_id, value, metadata in node:
        ...
        with tracer.start_as_current_span(
            ...
            if angle < np.pi / 2 and angle > -np.pi / 2:
                ...
                target.channels[1] = target_rotation
                target.channels[0] = TARGET_SPEED
            elif angle < -np.pi / 2:
                target_rotation = 1000
                target.channels[1] = target_rotation
                target.channels[0] = TARGET_SPEED
            else:
                target.channels[1] = 2000
                target.channels[0] = TARGET_SPEED
    ...
    print("stopping")
    target = OverrideRCIn()
    target.channels[1] = 2000 # right turn stop
    target.channels[0] = 1000 # Stop channel value
    print(target)
    pub.publish(target)
    print("stopped")
